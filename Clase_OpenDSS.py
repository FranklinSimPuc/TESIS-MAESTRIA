class DSS():
        ...
        def Fit(self,X1,X2,X3,estado):
            self.compile_DSS()
            if (estado==1 or estado=='1'):
                self.dssText.Command = "SwtControl."+"S"+str(X1)+".action=close"
            elif (estado==0 or estado=='0'):
                self.dssText.Command = "SwtControl."+"S"+str(X1)+".action=open"
            if (estado==1 or estado=='1'):
                self.dssText.Command = "SwtControl."+"S"+str(X2)+".action=close"
            elif (estado==0 or estado=='0'):
                self.dssText.Command = "SwtControl."+"S"+str(X2)+".action=open"
            if (estado==1 or estado=='1'):
                self.dssText.Command = "SwtControl."+"S"+str(X3)+".action=close"
            elif (estado==0 or estado=='0'):
                self.dssText.Command = "SwtControl."+"S"+str(X3)+".action=open"
            self.dssSolution.Solve()
            Perdas,Perdas2 = self.dssCircuit.LineLosses
            return Perdas

        def FitnessIni(self,SolIni,estado):
            tamanho=len(SolIni)
            Perdidas=[0]*tamanho
            mallas=len(SolIni[0])
            MejorSolucion=SolIni[0].copy()
            PerdidaMinima=9999999999999         
            #print(tamanho,'-',Perdidas,'-',mallas)
            for i in range(tamanho):
                X=SolIni[i].copy()
                #print(X)
                self.compile_DSS()
                for j in range(mallas):
                    XX=str(X[j])
                    #print(XX)
                    if (estado==1 or estado=='1'):
                        self.dssText.Command = "SwtControl."+"S"+XX+".action=close"
                    elif (estado==0 or estado=='0'):
                        self.dssText.Command = "SwtControl."+"S"+XX+".action=open"
                self.dssSolution.Solve()

                TenMin = min(self.dssCircuit.AllBusVmag)
                Perdas,Perdas2 = self.dssCircuit.LineLosses
                Perdidas[i]=Perdas
                if (Perdas<PerdidaMinima and TenMin>0.0001):
                    PerdidaMinima=Perdas
                    MejorSolucion=SolIni[i].copy()
            return Perdidas,PerdidaMinima,MejorSolucion
        
        def FitnessVecino(self,X,estado):
            tamanho=len(X)
            self.compile_DSS()
            for i in range(tamanho):
                XX=str(X[i])
            ...
            TenMin=min(self.dssCircuit.AllBusVmag)
            Perdas,Perdas2 = self.dssCircuit.LineLosses
            return TenMin,Perdas         


        def Fitness(self,X1,X2,X3,estado):
            tamanho=len(X1)
            Perdidas=[0]*tamanho
            for i in range(tamanho):
                self.compile_DSS()
                ...
                XX3=str(X3[i])
                if (estado==1 or estado=='1'):
                    self.dssText.Command = "SwtControl."+"S"+XX1+".action=close"
                elif (estado==0 or estado=='0'):
                    self.dssText.Command = "SwtControl."+"S"+XX1+".action=open"
                if (estado==1 or estado=='1'):
                    self.dssText.Command = "SwtControl."+"S"+XX2+".action=close"
                elif (estado==0 or estado=='0'):
                    self.dssText.Command = "SwtControl."+"S"+XX2+".action=open"
                if (estado==1 or estado=='1'):
                    self.dssText.Command = "SwtControl."+"S"+XX3+".action=close"
                elif (estado==0 or estado=='0'):
                    self.dssText.Command = "SwtControl."+"S"+XX3+".action=open"
                self.dssSolution.Solve()
                Perdas,Perdas2 = self.dssCircuit.LineLosses
                Perdidas[i]=Perdas
            return Perdidas
        
        def Fit33(self,X1,X2,X3,X4,X5,estado):
            self.compile_DSS()
            if (estado==1 or estado=='1'):
                self.dssText.Command = "SwtControl."+"S"+str(X1)+".action=close"
            elif (estado==0 or estado=='0'):
            ...
            if (estado==1 or estado=='1'):
                ...
                self.dssText.Command = "SwtControl."+"S"+str(X3)+".action=open"
            if (estado==1 or estado=='1'):
                self.dssText.Command = "SwtControl."+"S"+str(X4)+".action=close"
            elif (estado==0 or estado=='0'):
                self.dssText.Command = "SwtControl."+"S"+str(X4)+".action=open"
            if (estado==1 or estado=='1'):
                self.dssText.Command = "SwtControl."+"S"+str(X5)+".action=close"
            elif (estado==0 or estado=='0'):
                self.dssText.Command = "SwtControl."+"S"+str(X5)+".action=open"
            self.dssSolution.Solve()
            Perdas,Perdas2 = self.dssCircuit.LineLosses
            return Perdas
